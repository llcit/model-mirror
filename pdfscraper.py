...
numPassages = str(num)
text = 'QUERY: write ' + numPassages + ' 120-word passages in' + language + ' about this exerpt: ' + c
initInstruction = 'You are a language teacher tasked with developing 120 word passages for the purpose of language instruction. These passages will be used to test language learners with their corresponding literacy levels. Generate unique passages as a base for this learning process.'
...
beginnerInstruction = 'Simplify the following passages to beginner level, which means the language used has short sentences, simple grammar patterns, and uses vocabulary words of high frequency. Separate these passages with #### as a delimiter'
